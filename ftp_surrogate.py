...
class IcebergJob:
    def __init__(self, spark, glue_context, args):  
        ...
        self.partition_column = [t.strip() for t in args['partition_column'].split(',')] if 'partition_column' in args else None
    ...
    def run(self):
        ...
        for idx, (table_name, s3_path, warehouse_path) in enumerate(zip(self.table_names, self.s3_paths, self.warehouse_path)):
            try:
                ...
                df = self._add_surrogate_key(df)
                logger.info("Added surrogate key column using all columns in DataFrame.")
    ...
    def _validate_partition_column(self, df):
        if self.partition_column:
            if isinstance(self.partition_column, str):
            ...
            elif isinstance(self.partition_column, list):
                missing_columns = [col for col in self.partition_column if col not in df.columns]               
                if missing_columns:
                    raise ValueError(f"Partition columns {missing_columns} not found in input data")
                else:
                    logger.info(f"Partition columns {self.partition_column} validated")
    ...
    def _create_table(self, df, table_name, warehouse_path):
        ...
        if self.partition_column:
            if isinstance(self.partition_column, list):
