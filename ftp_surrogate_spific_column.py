...
class IcebergJob:
    def __init__(self, spark, glue_context, args):  
        ...
        self.module_name_path = args['module_name_path'].strip() if 'module_name_path' in args else None
    ...
    def run(self):
        ...
        config = self.load_json_from_s3(self.module_name_path)
        desired_columns = config['columns']
        num_columns = len(desired_columns)
        ...
        for idx, (table_name, s3_path, warehouse_path) in enumerate(zip(self.table_names, self.s3_paths, self.warehouse_path)):
            try:
                ...
                self.load_json_from_s3(self.module_name_path)
                df = self._read_data(s3_path, desired_columns)
                self._check_table_exists(self.database_name, table_name)
                self._ensure_table_ready(self.database_name, table_name)
    ...
    def load_json_from_s3(self, module_name_path):
        try:
            s3 = boto3.client('s3')
            bucket = module_name_path.split('/')[2]
            key = '/'.join(module_name_path.split('/')[3:])
            s3_object = s3.get_object(Bucket=bucket, Key=key)
            json_str = s3_object['Body'].read().decode('utf-8')
            config = json.loads(json_str)
            logger.info(f"Loaded schema config from {module_name_path} with {len(config.get('columns', []))} columns.")
            return config
        except Exception as e:
            logger.critical(f"Failed to load JSON from {module_name_path}: {e}", exc_info=True)
            raise

    def _read_data(self, s3_path, desired_columns):
        logger.info(f"Reading data from S3 path: {s3_path}")
        
        df = self.spark.read.format("csv").load(s3_path)
        num_columns = len(desired_columns)
        subset_cols = [f"_c{i}" for i in range(num_columns)]
        df_subset = df.select(*subset_cols)
        df_subset = df_subset.toDF(*desired_columns)
        if "Time" in desired_columns:
            df_subset = df_subset.withColumn("Time", regexp_replace("Time", "/", ""))
        row_count = df_subset.count()
        logger.info(f"Read {row_count} rows from {s3_path}, subset with {num_columns} columns")
        if row_count == 0:
